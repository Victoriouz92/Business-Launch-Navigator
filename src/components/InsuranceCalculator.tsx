 ...
 * - Минимален осигурителен доход за самоосигуряващи се: €510/мес (= мин. работна заплата)
 * - Ако заплатата на трудов договор >= максимума → доплащане = €0
 * - Ако заплатата < максимума → самоосигуряваш се върху разликата (мин. €480)
 * - Осигурителни вноски за самоосигуряващ се (ДОО+ЗО): ~31.3% от дохода
 */

const MAX_INSURANCE_INCOME = 2300 // €/мес (от 01.08.2026, бюджет ДОО 2026)
const MIN_SELF_EMPLOYED_INCOME = 510 // €/мес (= минимална работна заплата 2026)
const CONTRIBUTION_RATE = 0.313 // ДОО 25.8% + ЗО 8% = ~31.3%
  ...
  return (
    <div className="rounded-xl border-2 border-indigo-200 dark:border-indigo-800 bg-indigo-50/50 dark:bg-indigo-950/20 p-5">
        ...
        <div className="mt-4 rounded-lg bg-white dark:bg-slate-800 border border-indigo-100 dark:border-indigo-900 p-4 space-y-2">
          ...
          {result.monthly > 0 && (
            <p className="text-xs text-slate-500 dark:text-slate-400">
              ...
              <br />
              Макс. осигурителен доход 2025: €{MAX_INSURANCE_INCOME}/мес
            </p>
          )}
          {result.monthly === 0 && (
      ...
      <p className="mt-3 text-xs text-slate-400 dark:text-slate-500">
        * Приблизителна стойност. Данни за 2026 г. (бюджет на ДОО, приет на първо четене юли 2026). Консултирай се със счетоводител за точната сума.
      </p>
    </div>
  )
}
